...
hgeParticleSystem* hgeParticleManager::SpawnPS(const char *filename, DDImage *sprite, float x, float y)
{
	if(nPS==MAX_PSYSTEMS)
	{
		MATCHBLOX_ASSERT(true, "Max particle systems reached.");
		return 0;
	}
	psList[nPS]=new hgeParticleSystem(filename, sprite,fFPS);
