...
const std::string PLANET_EXPORTS[] = 
...
void Board::UpdateF(float theFrac)
{
	...
	if ((!mLevelupEffect->HidePlanets() || (gPauseLevel > 0)) && !mGameOverEffect->IsActive())
	{
		// Move the planets
...
void Board::Draw(Graphics* g)
{
	...
	if (!mLevelupEffect->HideBoard() && !mGameOverEffect->HideBoard())
	{
		...
		if (!mLevelupEffect->HidePlanets() || (gPauseLevel > 0))
			DrawPlanets(g);
	...
	if (gPauseLevel > 0)
	{
		...
		g->DrawString("PAUSED", mWidth / 2 - FONT_DEFAULT->StringWidth("PAUSED") / 2, mHeight / 2);
	}
}
...
void Board::DrawUI(Graphics* g)
{
	...
	g->SetColor(Color(255, 0, 0, 200));
	g->DrawString(StrFormat("%d%%", mPercentComplete), x + FONT_DEFAULT->StringWidth(s), ascent);
...
{	
	...
	mOptionsBtn->Resize(gSexyAppBase->mWidth - IMAGE_BUTTON_NORMAL->GetWidth() - 10, FONT_DEFAULT->GetHeight() * 3 - 20, 
...
{		
	...
	if (mLevelupEffect->IsActive() || (gPauseLevel > 0) || (mGameOverEffect->IsActive() && !mGameOverEffect->CanStartNewGame()))
		return;
...
void Board::GivePlanetBonus(Planet* p)
{
	...
	std::string pName = PLANET_NAME[p->mNameIdx];
	std::string pExport = PLANET_EXPORTS[p->mExportIdx];
	int points = mLevel * 1000;
	AddBonusText(StrFormat("%s: +%d", pName.c_str(), points), p->mX, p->mY);
	mScore += points;

	mPlanetsEaten.push_back(pName.c_str());
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void Board::AddBonusText(std::string t)
{
	AddBonusText(t, mWidth / 2 - FONT_DEFAULT->StringWidth(t) / 2, 
				    (mHeight - GRID_START_Y) / 2 - FONT_DEFAULT->GetHeight() / 2);
}

//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void Board::AddBonusText(std::string t, float x, float y)
{
	BonusText bt;
	bt.mText = t;
	bt.mX = x;
