...
void LevelupEffect::Init()
{
	...
	//	by a height of FONT_DEFAULT->GetHeight() pixels. Each letter will initially
	...
	float x = (GRID_END_X - GRID_START_X) / 2 - FONT_DEFAULT->StringWidth("LEVEL UP!") / 2;
	float y = FONT_DEFAULT->GetHeight();
	float startingY = y;
	float ydec = y;
	float speed = 5.0f;
	...
	x += FONT_DEFAULT->StringWidth("L");
	y -= ydec;
	mText.push_back(BouncyChar("E", x, y, speed));

	x += FONT_DEFAULT->StringWidth("E");
	y -= ydec;
	mText.push_back(BouncyChar("V", x, y, speed));

	x += FONT_DEFAULT->StringWidth("V");
	y -= ydec;
	mText.push_back(BouncyChar("E", x, y, speed));
	...
	x += FONT_DEFAULT->StringWidth("U");
	y -= ydec;
	mText.push_back(BouncyChar("P", x, y, speed));

	x += FONT_DEFAULT->StringWidth("P");
	y -= ydec;
...
void LevelupEffect::Draw(Graphics* g)
{
				...
				(mState == LevelupEffect::COVERING_STATS))
	{
		...
		int y = 50;
		g->SetColor(gSexyAppBase->HSLToRGB(mHue, 255, 128) & 0xFFFFFFFF);
		std::string s = StrFormat("LEVEL %d COMPLETE!", mStats.mLevelCompleted);
		g->DrawString(s, gSexyAppBase->mWidth / 2 - FONT_DEFAULT->StringWidth(s) / 2, y);

		g->SetColor(Color::White);
		s = "POPULATION CONSUMED:";
		int strWidth = FONT_DEFAULT->StringWidth(s);
		...
		y += FONT_DEFAULT->GetHeight();
		g->SetColor(Color::White);		
		s = "SYSTEMS SUBJUGATED:";
		strWidth = FONT_DEFAULT->StringWidth(s);
		g->DrawString(s, rightX - strWidth, y);
		g->SetColor(Color(255, 0, 0));
		g->DrawString(StrFormat("%d%%", mStats.mPartUncovered), rightX + 5, y);
		
		y += FONT_DEFAULT->GetHeight();
		if (mStats.mPartUncovered >= COMPLETION_BONUS_PCT)
		{
		...
		if (mStats.mPlanetsEaten.size() > 0)
		{
			...
			s = "PLANET EATEN:";
			g->DrawString(s, third / 2 - FONT_DEFAULT->StringWidth(s) / 2, y);
			s = "EXPORTS:";
			g->DrawString(s, third + (third / 2 - FONT_DEFAULT->StringWidth(s) / 2), y);
			s = "POPULATION:";
			g->DrawString(s, third*2 + (third / 2 - FONT_DEFAULT->StringWidth(s) / 2), y);
			...
			{
				if (y >= gSexyAppBase->mHeight - FONT_DEFAULT->GetHeight() * 2)
				{
					drawDotDotDot = true;
					break;
				}
		...
		g->SetColor(Color::White);
		s = "PLAY NOW";
		g->DrawString(s, gSexyAppBase->mWidth / 2 - FONT_DEFAULT->StringWidth(s) / 2, gSexyAppBase->mHeight - 20);		
	}
